...
def main(arguments):
    ...
    items = filter(
        lambda item: item[0] in (prayers[args.lang] if not args.compact else prayers_compact[args.lang]),
        prayers_info["data"]["timings"].items(),
    )
    items = [
        (
    ...
    if args.next:
        ...
        if next_prayers:
            ...
            print(
                next_prayer[0],
                next_prayer[1].strftime("%-I:%M%P" if not args.compact else "%-I:%M"),
                f"({remain[args.lang] if not args.compact else ''}{':'.join(str(time_remaining).split(':')[:2])})",
            )
    else:
